...
import { AlertCircle, ArrowLeft, Calendar, Car, CreditCard, FileText, Mail, Phone, Printer, User } from 'lucide-react';
...
export default function Show({
    ...
    const companiaNombre = (orden as any).compania_seguro?.nombre ?? 'Sin seguro / Particular';
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2 text-gray-600">
                                ...
                                <span className="rounded-full border border-yellow-200 bg-yellow-100 px-2.5 py-0.5 text-xs font-semibold text-yellow-700">
                                    {orden.estado.nombre}
                                </span>
                                    ...
                                    <table className="w-full text-left">
                                        <thead>
                                            <tr className="border-b border-gray-100 text-xs font-semibold tracking-wider text-gray-500 uppercase">
                                                ...
                                                {canViewFinancialAmounts && <th className="pr-2 pb-3 text-right">Subtotal</th>}
                                            </tr>
                                        </thead>
                                            ...
                                            {orden.detalles.map((detalle, index) => (
                                                <tr key={index} className="text-sm text-gray-700">
                                                    <td className="py-3 pl-2">
                                                        ...
                                                        {detalle.descripcion && (
                                                            <div className="mt-1 text-sm text-gray-500">{detalle.descripcion}</div>
                                                        )}
                                                    </td>
                                        ...
                                        {canViewFinancialAmounts && totalOrden !== null && (
                                            <tfoot>
                                                <tr className="border-t border-gray-100">
                                                    ...
                                                    <td className="pt-4 pr-2 text-right text-lg font-bold text-green-600">
                            ...
                            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                                ...
                                <div className="space-y-6 p-6">
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                                        ...
                                        <SummaryCard
                                            label="Total cobrado"
                                            value={totalPagado !== null ? formatMoney(totalPagado) : 'Oculto'}
                                            accent="green"
                                        />
                                        <SummaryCard
                                            ...
                                            {orden.pagos.map((pago) => (
                                                ...
                                                >
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-gray-500">{formatDateToArgentina(pago.fecha)}</p>
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="mt-1 text-sm text-amber-700">
                                                Este rol puede continuar con la gestión operativa de la orden, pero no visualiza montos, cobros ni
                                                totales.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {orden.historial_estados.map((h, index) => (
                                        <div key={h.id} className="relative pl-6">
                                            {index !== orden.historial_estados!.length - 1 && (
                                                <div className="absolute top-4 bottom-0 left-2 w-px bg-gray-200"></div>
                                            )}
                                            <div className="absolute top-1.5 left-0 h-4 w-4 rounded-full border-4 border-white bg-green-500 shadow"></div>
                                            <div className="ml-4">
                    ...
                    <div className="space-y-6">
                        ...
                        <SideCard title="Vehículo" icon={<Car className="h-5 w-5 text-gray-500" />}>
                            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 text-center">
                                <p className="text-xs tracking-wider text-gray-500 uppercase">Patente</p>
